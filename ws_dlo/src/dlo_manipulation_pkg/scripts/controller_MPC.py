...
class Controller(object):
    ...
    def generate_MPC_solver(self):
        ...
        C1 = ca.SX.sym('C1', (1, 12))
        C2 = ca.SX.sym('C2', (6, 12))
        ...
        for i in range(window_length):
            ...
            dot_x_i = Jacobian @ u_aug
            
            # Objective function
            state_err = X[:, i+1] - xd
            # err += U[:, i].T @ weight @ U[:, i] + 10*state_err.T @ state_err #+ dot_x_i.T @ dot_x_i # real system cost function
            err += u_aug.T @ u_aug + state_err.T @ state_err #+ dot_x_i.T @ dot_x_i # simulation cost function
            #Constraints
        ...
        for i in range(window_length):
            ...
            g_C2 = ca.vertcat(g_C2, C2@u_aug)
            
        g = ca.vertcat(g, g_C2, g_C1)
        ...
        p = ca.vertcat(x0, xd, ca.vec(Jacobian), ca.vec(C1), ca.vec(C2))
    ...
    def generateControlInput(self, state):
        ...
        C1, C2 = get_matrices_C1_C2(state)
